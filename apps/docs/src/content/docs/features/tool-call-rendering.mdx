...
If you need additional fields (e.g. `tool_call_id`, `status`), or your backend sends custom event types beyond `text_delta` and `tool_call`, see [Custom Event Handling](/extending/custom-events) for extending both part types and event types with generics.
