...
By default, `useChat` concatenates all `delta` values into the assistant message's `content`.
...
These are **not** handled by default. Use the [`onEvent` callback](/extending/custom-events) to process them.
